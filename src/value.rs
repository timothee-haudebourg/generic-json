	...
	T::Object: PartialOrd<U::Object>
{
	fn partial_cmp(&self, other: &Value<U>) -> Option<Ordering> {
		match (self, other) {
			...
			(Self::String(_), Value::Null | Value::Boolean(_) | Value::Number(_)) => Some(Ordering::Greater),
			(Self::String(a), Value::String(b)) => a.partial_cmp(b),
			(Self::String(_), _) => Some(Ordering::Less),
			(Self::Array(_), Value::Null | Value::Boolean(_) | Value::Number(_) | Value::String(_)) => Some(Ordering::Greater),
...
impl<'a, T: Json> From<&'a str> for Value<T> where T::String: From<&'a str> {
	fn from(s: &'a str) -> Self {
		Self::String(s.into())
	}
}

impl<T: Json> From<String> for Value<T> where T::String: From<String> {
	fn from(s: String) -> Self {
		Self::String(s.into())
	}
}

impl<'a, T: Json> From<Cow<'a, str>> for Value<T> where T::String: From<Cow<'a, str>> {
	fn from(s: Cow<'a, str>) -> Self {
		Self::String(s.into())
	}
}

impl<'a, T: Json> From<&'a [Value<T>]> for Value<T> where T::Array: From<&'a [Value<T>]> {
	fn from(a: &'a [Value<T>]) -> Self {
		Self::Array(a.into())
	}
}

impl<T: Json> From<Vec<Value<T>>> for Value<T> where T::Array: From<Vec<Value<T>>> {
	fn from(a: Vec<Value<T>>) -> Self {
		Self::Array(a.into())
	}
}

impl<T: Json, V: Into<Self>> FromIterator<V> for Value<T> where T::Array: FromIterator<Self> {
